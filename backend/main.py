...
YOUTUBE_CLIENTS = [x.strip() for x in os.getenv("YOUTUBE_CLIENTS", "mweb").split(",") if x.strip()]
YTDLP_JS_RUNTIME = os.getenv("YTDLP_JS_RUNTIME", "deno").strip() or "deno"
YTDLP_SLEEP_REQUESTS = os.getenv("YTDLP_SLEEP_REQUESTS", "0.75").strip() or "0.75"
...
def is_youtube_bot_block(message: str) -> bool:
...
def youtube_block_message() -> str:
    return (
        "YouTube is blocking requests from this backend server's public IP. "
        "The PO-token provider is configured, but PO tokens cannot guarantee that a flagged "
        "datacenter IP will pass YouTube's bot checks. Try again later or run the backend from "
        "a different network/host whose public IP is not being challenged by YouTube."
    )
...
def run_youtube(extra_args: list[str], url: str, timeout: int) -> subprocess.CompletedProcess:
    ...
    for client in YOUTUBE_CLIENTS:
        try:
            ...
            message = str(e)
            if is_youtube_bot_block(message):
                raise RuntimeError(youtube_block_message())
            errors.append(f"{client}: {message[-900:]}")
    detail = " | ".join(errors[-3:])
    raise RuntimeError(
        "YouTube extraction failed after trying the configured client(s). "
        + detail
    )
...
def health():
    return {
        ...
        "js_runtime_version": command_version([YTDLP_JS_RUNTIME, "--version"]),
